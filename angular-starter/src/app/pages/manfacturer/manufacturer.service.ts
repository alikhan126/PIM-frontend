...
getAllHidden() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_Manfracturer + "unapproved/" }`)
  .pipe(
    map(x => x ),
    tap(_ => console.log('fetched record')),
    catchError(this.handleError('getRecord', []))
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_Manfracturer}${id}/`).pipe(
    map(x => x ),
    tap(_ => console.log(`get record=${id}`)),
    catchError(this.handleError<any>('getRecord'))
  );
...
//   const url = `${AppConfig.URL_Manfracturer}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_Manfracturer}${record["id"]}/`, record).pipe(
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_Manfracturer}${id}/`).pipe(
    map(x => x ),
