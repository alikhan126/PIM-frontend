...
@Component({
    ...
    templateUrl: './manufacturer-hidden.component.html',
    styleUrls: ['./manufacturer-hidden.component.scss']
})

export class ManufacturerHiddenComponent {
    editing = {};
    rows = [];
    roles = [];
    products :any[];
    ...
    approveManufacturer(event, cell, rowIndex) {
        let user=JSON.parse(localStorage.getItem('currentUser'));
        if (user.is_admin == false){ 
            ...
            this.manufacturerService.get(this.rows[rowIndex]['id']).subscribe(data => {
                let result = data;
                result['hidden'] = false
                this.manufacturerService.update(result).subscribe(data => {
                    this.manufacturerService.getAllHidden().subscribe(data => {
                        this.rows = data;
                    });
                });
