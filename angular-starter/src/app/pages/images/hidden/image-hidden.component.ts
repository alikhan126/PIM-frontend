...
@Component({
    ...
    templateUrl: './image-hidden.component.html',
    styleUrls: ['./image-hidden.component.scss']
})

export class ImageHiddenComponent {
    editing = {};
    rows = [];
    roles = [];
    products :any[];
    ...
    constructor(private modalService: NgbModal, private imageService: ImageService, private router: Router) {
    }
    ngOnInit() {
        this.getUserRole();
        this.imageService.getAllHidden().subscribe(data => {
            this.rows = data;
            this.temp = data;
    ...
    approveTag(event, cell, rowIndex) {
        ...
        if (user.is_admin == false){ 
            ...
            this.imageService.get(this.rows[rowIndex]['id']).subscribe(data => {
                let result = data;
                result['hidden'] = false
                this.imageService.update(result).subscribe(data => {
                    this.imageService.getAllHidden().subscribe(data => {
                        this.rows = data;
                    });
                });
