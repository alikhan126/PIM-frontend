...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_Images}`, record).pipe(
    map(x => x ),
...
getAllHidden() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_Images + "unapproved/" }`)
  .pipe(
    map(x => x ),
    tap(_ => console.log('fetched record')),
    catchError(this.handleError('getRecord', []))
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_Images}${id}/`).pipe(
    map(x => x ),
    tap(_ => console.log(`get record=${id}`)),
    catchError(this.handleError<any>('getRecord'))
  );
...
//   const url = `${AppConfig.URL_Images}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_Images}${record["id"]}/`, record).pipe(
    tap(_ => this.log(`updated record id=${record["id"]}`)),
    catchError(this.handleError<any>('updateRecord'))
  );
}
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_Images}${id}/`).pipe(
    map(x => x ),
