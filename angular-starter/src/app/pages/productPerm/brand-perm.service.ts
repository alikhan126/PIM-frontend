...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_BrandPermission}`, record).pipe(
    map(x => x ),
...
getAll() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_BrandPermission}`)
  .pipe(
    map(x => x ),
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_BrandPermission}${id}/`).pipe(
    map(x => x ),
...
//   const url = `${AppConfig.URL_BrandPermission}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_BrandPermission}${record["id"]}/`, record).pipe(
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_BrandPermission}${id}/`).pipe(
    map(x => x ),
