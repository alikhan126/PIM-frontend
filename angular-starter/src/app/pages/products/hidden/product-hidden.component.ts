...
@Component({
    ...
    templateUrl: './product-hidden.component.html',
    styleUrls: ['./product-hidden.component.scss']
})

export class ProductsHiddenComponent implements OnInit{
...
saveAsCatalog(){
    ...
    getData(){
        this.productService.getAllHidden().subscribe(data => {
            this.rows = data;
            this.temp = data;
        });
    }
    ...
    approveProduct(event, cell, rowIndex) {
        let user=JSON.parse(localStorage.getItem('currentUser'));
        if (user.is_admin == false){ 
            alert("Only admin can approve this product.");
        } else {
            this.editing[rowIndex + '-' + cell] = false;
            this.productService.get(this.rows[rowIndex]['id']).subscribe(data => {
                let result = data;
                result['hidden'] = false
                this.productService.update(result).subscribe(data => {
                    this.productService.getAllHidden().subscribe(data => {
                        this.rows = data;
                    });
                });
