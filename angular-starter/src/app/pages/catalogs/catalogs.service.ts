...
getFieldPermissions(id){
  return this.http.get<any>(`${AppConfig.URL_WebsitePermissionCheck + "?role="}${id}`).pipe(
    map(x => x ),
...
getAll() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_CATALOGS}`)
  .pipe(
    map(x => x ),
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_CATALOGS}${id}/`).pipe(
    map(x => x ),
    tap(_ => console.log(`get record=${id}`)),
    catchError(this.handleError<any>('getRecord'))
  );
...
//   const url = `${AppConfig.URL_CATALOGS}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_CATALOGS}${record["id"]}/`, record).pipe(
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_CATALOGS}${id}/`).pipe(
    map(x => x ),
...
destroy (record: any): Observable<any> {
  console.log(record)
  return this.http.delete(`${AppConfig.URL_CATALOGS}${record["id"]}/`, record).pipe(
