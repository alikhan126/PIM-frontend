...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_Roles}`, record).pipe(
    map(x => x ),
...
getAll() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_Roles}`)
  .pipe(
    map(x => x ),
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_Roles}${id}/`).pipe(
    map(x => x ),
...
//   const url = `${AppConfig.URL_Roles}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_Roles}${record["id"]}/`, record).pipe(
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_Roles}${id}/`).pipe(
    map(x => x ),
