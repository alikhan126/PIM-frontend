...
@Component({
    ...
    templateUrl: './website-hidden.component.html',
    styleUrls: ['./website-hidden.component.scss']
})

export class WebsitesHiddenComponent {
    editing = {};
    rows = [];
    roles = [];
    products :any[];
    ...
    ngOnInit() {
        this.getUserRole();
        this.websiteService.getAllHidden().subscribe(data => {
            this.rows = data;
            this.temp = data;
    ...
    approveWebsite(event, cell, rowIndex) {
        let user=JSON.parse(localStorage.getItem('currentUser'));
        if (user.is_admin == false){ 
            ...
            this.websiteService.get(this.rows[rowIndex]['id']).subscribe(data => {
                let result = data;
                result['hidden'] = false
                this.websiteService.update(result).subscribe(data => {
                    this.websiteService.getAllHidden().subscribe(data => {
                        this.rows = data;
                    });
                });
