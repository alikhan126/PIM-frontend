...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_Websites}`, record).pipe(
    map(x => x ),
...
getAllHidden() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_Websites + "unapproved/" }`)
  .pipe(
    map(x => x ),
    tap(_ => console.log('fetched record')),
    catchError(this.handleError('getRecord', []))
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_Websites}${id}/`).pipe(
    map(x => x ),
    tap(_ => console.log(`get record=${id}`)),
    catchError(this.handleError<any>('getRecord'))
  );
...
//   const url = `${AppConfig.URL_Websites}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_Websites}${record["id"]}/`, record).pipe(
