...
@Component({
    ...
    templateUrl: './video-hidden.component.html',
    styleUrls: ['./video-hidden.component.scss']
})
...
export class ImageHiddenComponent {
    ...
    constructor(private modalService: NgbModal, private videoService: VideoService, private router: Router) {
    }
    ngOnInit() {
        this.getUserRole();
        this.videoService.getAllHidden().subscribe(data => {
            this.rows = data;
            this.temp = data;
    ...
    approveTag(event, cell, rowIndex) {
        ...
        if (user.is_admin == false){ 
            ...
            this.videoService.get(this.rows[rowIndex]['id']).subscribe(data => {
                let result = data;
                result['hidden'] = false
                this.videoService.update(result).subscribe(data => {
                    this.videoService.getAllHidden().subscribe(data => {
                        this.rows = data;
                    });
                });
