...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_Role}`, record).pipe(
    map(x => x ),
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_Role}${id}/`).pipe(
    map(x => x ),
...
//   const url = `${AppConfig.URL_Role}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_Role}${record["id"]}/`, record).pipe(
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_Role}${id}/`).pipe(
    map(x => x ),
