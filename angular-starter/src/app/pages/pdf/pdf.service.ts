...
add (record: any): Observable<any> {
  ...
  return this.http.post<any>(`${AppConfig.URL_PDF}`, record).pipe(
    map(x => x ),
...
getAll() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_PDF}`)
  .pipe(
    map(x => x ),
...
getAllHidden() 
{
  ...
  return this.http.get<any []>(`${AppConfig.URL_PDF + "unapproved/" }`)
  .pipe(
    map(x => x ),
    tap(_ => console.log('fetched record')),
    catchError(this.handleError('getRecord', []))
...
get(id){
  return this.http.get<any>(`${AppConfig.URL_PDF}${id}/`).pipe(
    map(x => x ),
    tap(_ => console.log(`get record=${id}`)),
    catchError(this.handleError<any>('getRecord'))
  );
...
//   const url = `${AppConfig.URL_PDF}${id}`;
...
update (record: any): Observable<any> {
  return this.http.put(`${AppConfig.URL_PDF}${record["id"]}/`, record).pipe(
    tap(_ => this.log(`updated record id=${record["id"]}`)),
    catchError(this.handleError<any>('updateRecord'))
  );
}
...
delete(id){
  return this.http.delete<any>(`${AppConfig.URL_PDF}${id}/`).pipe(
    map(x => x ),
